...
// Nehahra - End
	...
	if (r_interpolate_model_transform.value && !e->torch && (client_no != cl.viewentity || !chase_active.value || !cls.demoplayback))
...
// Nehahra - End
	...
	if (e->torch || (clmodel->candle || clmodel->lantern) && gl_autobright.value ||
	    clmodel->bolt || clmodel->missile || clmodel->quaddama || clmodel->invulner)
		e->glow = true; // Glow is now done with trans ents
}
...
void R_DrawGlow (entity_t *e)
{
	...
	if (gl_fogenable.value && !gl_fogdisable.value)
	{
		...
		// Attenuate radius in fog with distance to avoid weird halos.
		// Start attenuation at distance att_begin
	...
	if (radius && distance > radius)
	{
		...
		// Invert (fades as you approach) - if we are a torch
                if (e->torch || clmodel->candle || clmodel->lantern)
		{
...
void R_DrawTransEntities (void)
{
	...
	for (i=j=0 ; i<cl_numvisedicts ; i++)
	{
		...
		// Glow might be set here already in a non-transparent entity
		if (!currententity->glow && (currententity->transparency == 1 || currententity->transparency == 0))
			continue;
	...
	for (i = 0; i < j; ++i)
	{
		...
		{
		case mod_alias:
			// Glow might get set here in a transparent entity
