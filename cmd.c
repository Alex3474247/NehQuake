...
============
*/
void Cbuf_AddText2 (char *text)
{
	if (*text == 'p' && !strncmp(text, "playdemo ", 9))
	{
		if (cmd_text.cursize > 0 && cmd_text.data[cmd_text.cursize - 1] != '\n')
		{
			// Gross hack to parse corrupt demos
			Cbuf_AddText ("\n");
			Con_DPrintf ("Cbuf_AddText2: possibly corrupt stufftext found\n");
		}
		
		SCR_BeginLoadingPlaque (); // Ease transition to next demo
	}
...
// pull out the commands
	...
	for (i=0 ; i<s-1 ; i++)
	{
		if (text[i] == '+')
		{
			...
			for (j=i ; (text[j] != '+') && (text[j] != '-' || j > i && text[j - 1] != ' ') && (text[j] != 0) ; j++)
				;
...
void Cmd_Alias_f (void)
{
	...
	if (Cmd_Argc() == 1)
	{
		Con_SafePrintf ("Current alias commands:\n");
...
// copy the rest of the command line
	...
	a->value = Z_Malloc (len + 1);

	for (i=2 ; i< c ; i++)
	{
		strcat (a->value, Cmd_Argv(i));
		if (i != c)
			strcat (a->value, " ");
	}
	strcat (a->value, "\n");
