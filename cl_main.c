...
cvar_t	cl_altlerp = {"cl_altlerp","0"};
...
void NEH_ResetSFX (void)
{
	...
	num_sfx = num_sfxorig;                    // Nehahra
//  Con_DPrintf("Current SFX: %d\n", num_sfx);

	for (i = num_sfx ; i < MAX_SFX; i++)      // Trash known sfx
	{
		strcpy (known_sfx[i].name, "dfw3t23EWG#@T#@");
		if (known_sfx[i].cache.data)
			Cache_Free (&known_sfx[i].cache); // Release memory buffer
	}
}
...
// stop sounds (especially looping!)
	S_StopAllSounds (true);
        SHOWLMP_clear(); // Nehahra - Clear shown overlays
// bring the console down and fade the colors back to normal
//	SCR_BringDownConsole ();
        MOD_stop(); // Nehahra - stop playing module
...
// rotate binary objects locally
		...
		if (ent->model->flags) // LordHavoc: if the model has no flags, don't check each
		{
			...
			{
                                if (ent->transparency == -1) // LordHavoc: Nehahra dem compatibility
				{
...
int CL_ReadFromServer (void)
{
	...
	if (cl.time < 1)
	{
		// Kludge to reset at beginning of new map
	...
	{
		// Check old limit
		if (!WarnPrinted && cl.num_statics > 128)
		{
			WarnPrinted = true;
			Con_Printf ("\002CL_ReadFromServer: ");
			Con_Printf ("excessive static entities (%d, normal max = %d)\n", cl.num_statics, 128);
		}
	}
...
//
	...
	Cvar_RegisterVariable (&cl_altlerp);
