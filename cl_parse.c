...
//		static float lastmsg = 0;
		...
		if (sv.active || !r_interpolate_model_animation.value || !cl_altlerp.value ||
			...
			ent->forcelink = true;
//		else if (!ent->forcelink && IsTimeout (&lastmsg, 2))
//			Con_DPrintf ("CL_ParseUpdate: forcelink avoided\n");
	}
...
void CL_ParseClientdata (void)
{
	...
	if(cl.Protocol == PROTOCOL_VERSION_ALEXQUAKER)
	{
	if (bits & SU_EXTEND1)
...
//			Con_Printf ("svc_nop\n");
		...
		case svc_version:
			i = MSG_ReadLong ();
			if (i != PROTOCOL_VERSION && i != PROTOCOL_VERSION_ALEXQUAKER  &&  (i < PROTOCOL_VERSION_BJP || i > PROTOCOL_VERSION_BJP3))
				Host_Error ("CL_ParseServerMessage: Server is protocol %i instead of %i or %i-%i", i, PROTOCOL_VERSION, PROTOCOL_VERSION_BJP, PROTOCOL_VERSION_BJP3);//Alex, what the hell if this????
			cl.Protocol = i;
			break;
