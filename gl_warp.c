	...
	for (p=fa->polys ; p ; p=p->next)
	{
		...
		for (i=0,v=p->verts[0] ; i<p->numverts ; i++, v+=VERTEXSIZE)
		{
			...
			if(r_waterripple.value && !r_nowaterripple.value) {
				nv[0] = v[0];
				nv[1] = v[1];
				nv[2] = v[2] + r_waterripple.value*sin(v[0]*0.05+cl.time*3)*sin(v[2]*0.05+cl.time*3);
				glVertex3fv (nv);
			} else {
				glVertex3fv (v);
			}
...
//
	...
	count = columns * rows;
	count += 8; // If I don't add a few pixels here, validation LoadPCX3 below will sometimes fire (?)
...
void R_LoadSkys (char *skybox)
{
	...
	// First try gfx/env and then env
	for (j=0 ; j<2 ; j++)
	{
		for (i=0 ; i<6 ; i++)
		{
			sprintf (name, "%senv/%s%s", j == 0 ? "gfx/" : "", skybox, suf[suf_ord[i]]);
	...
	if (i == 6)
	{
		for (j = 0; j < 6; ++j)
		{
			...
			sprintf (name, "gfx/env/%s%s", skybox, suf[suf_ord[j]]);
			GL_Upload32 ((unsigned *)data[j], sky_width[j], sky_height[j], false, false, name);
		}
	}
	else
...
void R_SkyCommand_f (void)
{
	...
	case 2:
		...
		break;
	default:
		Con_Printf("usage: [load]sky <skyname>\n");
	}
}
...
static void MakeSkyVec (float s, float t, int axis)
{
	...
	// Compromise; big enough to avoid sky covering world for the Nehahra maps,
	// but small enough to avoid hiding skyboxes in fog
	farclip = gl_fogenable.value && !gl_fogdisable.value ? gl_skyclip.value : GL_FARCLIP;
