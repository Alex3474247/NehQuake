...
#include "quakedef.h"
#include "neh.h"
extern  cvar_t  mouselook, scr_fov, crosshair;
#ifdef _WIN32
#include "winquake.h"
#endif
...
enum {m_none, m_main, m_demo, m_singleplayer, m_load, m_save, m_multiplayer, m_setup, m_net, m_options, m_video, m_keys, m_help, m_quit, m_serialconfig, m_modemconfig, m_lanconfig, m_gameoptions, m_search, m_slist, m_vidopt} m_state;
void M_Menu_VidOpt_f ();
void M_VidOpt_Draw (void);    //Nehahra
void M_VidOpt_Key (int key);  //Nehahra
...
// Nehahra
int NumberOfDemos, DemoStart, DemoWindow;
typedef struct
{
 char name[50];
 char desc[50];
...
void M_Demo_Draw (void)
{
	...
	qpic_t	*p;
        ...
        for (i = 0; i < DemoWindow; i++)
		M_Print (16, 16 + 8*i, Demos[DemoStart + i].desc);
...
void M_Menu_Demos_f (void)
{
        ...
        NumberOfDemos = 34;
	DemoWindow = vid.height / 8 - 6;
	
	if (DemoWindow > NumberOfDemos)
		DemoWindow = NumberOfDemos;
...
void M_Demo_Key (int k)
{
  switch (k) {
    ...
    case K_ENTER:
     ...
     Cbuf_AddText (va ("playdemo %s\n", Demos[DemoStart + demo_cursor].name));
     return;
    ...
    case K_LEFTARROW:
     ...
     else
     {
	    if (DemoStart > 0)
		    --DemoStart;
	    else if (DemoWindow == NumberOfDemos)
	    {
		    DemoStart = NumberOfDemos - DemoWindow;
		    demo_cursor = DemoWindow - 1;
	    }
     }
     break;
    ...
    case K_RIGHTARROW:
     S_LocalSound ("misc/menu1.wav");
     if (demo_cursor + 1 < DemoWindow)
	    ++demo_cursor;
     else
     {
	    if (DemoStart < NumberOfDemos - DemoWindow)
		    ++DemoStart;
	    else if (DemoWindow == NumberOfDemos)
		    DemoStart = demo_cursor = 0;
     }
     break;
    }
}
...
static qboolean NehMenu;

void M_Menu_Main_f (void)
{
	if (!NehMenu)
		MAIN_ITEMS = 5; // Original menu
	else
	{
...
void M_Main_Draw (void)
{
	...
	// Nehahra
	if (!NehMenu || NehGameType == TYPE_BOTH)
...
void M_Main_Key (int key)
{
	...
	case K_ENTER:
		m_entersound = true;
                ...
                if (!NehMenu || NehGameType == TYPE_GAME)
		{
	...
	case K_ENTER:
		...
		case 0:
			...
			Cbuf_AddText ("maxplayers 1\n");
                        Cbuf_AddText (NehMenu ? "map nehstart\n" : "map start\n"); // Nehahra
			break;
...
void M_Menu_Help_f (void)
{
	if (NehMenu)
	{
		// No help in native Nehahra
		Con_Printf ("No help available\n");
		return;
	}
...
void M_Init (void)
{
	...
	// Check if Nehahra menu
//	COM_FindOnly ("gfx/mainmenu.lmp");
//	NehMenu = !stricmp (com_filedir, "nehahra");
	NehMenu = COM_FOpenFile ("gfx/mainmenu.lmp", &f) == 32648; // Kludge
...
// Enhanced menu
#define ENHANCED_ITEMS  11

int             enhanced_cursor;

void M_Menu_VidOpt_f (void)
{
	key_dest = key_menu;
        m_state = m_vidopt;
	m_entersound = true;
...
void M_VidOptAdjustSliders (int dir)
{
	float value;

	S_LocalSound ("misc/menu3.wav");

        switch (enhanced_cursor)
	{
	case 0:	// rocket trails
                Cvar_SetValue ("gl_rsmoke", !gl_rsmoke.value);
		break;
	case 1:	// chase mode
	...
	case 7:	// torch glows
		...
		break;
	case 8:	// cutscenes
                Cvar_SetValue ("cutscene", !cutscene.value);
		break;
        case 9: // water ripple
		value = r_waterripple.value;
                value += dir;
                if (value < 0)
                        value = 0;
                if (value > 10)
                        value = 10;
                Cvar_SetValue ("r_waterripple", value);
		break;
		...
		if (value > 170)
			value = 170;
		Cvar_SetValue ("fov", value);
		break;
	}
}

void M_VidOpt_Draw (void)
{
	float	r;
	qpic_t	*p;
	...
	p = Draw_CachePic ("gfx/ttl_cstm.lmp");
	M_DrawPic ( (320-p->width)/2, 4, p);
        M_Print (16, 32, "          Rocket Smoke");
        M_DrawCheckbox (220, 32, gl_rsmoke.value);
	...
	M_DrawCheckbox (220, 56, crosshair.value);

	M_Print (16, 64, "           Show Weapon");
	M_DrawCheckbox (220, 64, r_drawviewmodel.value);

	M_Print (16, 72, "     Smooth Animations");
	M_DrawCheckbox (220, 72, r_interpolate_model_animation.value);

	M_Print (16, 80, "       Smooth Movement");
	M_DrawCheckbox (220, 80, r_interpolate_model_transform.value);

        M_Print (16, 88, "              GL Glows");
        M_DrawCheckbox (220, 88, gl_glows.value);

        M_Print (16, 96, "             Cutscenes");
        M_DrawCheckbox (220, 96, cutscene.value);

        M_Print (16, 104, "          Water Ripple");
        r = (r_waterripple.value) / (10);
        M_DrawSlider (220, 104, r);

        M_Print (16, 112, "         Field of View");
	r = (scr_fov.value - 10) / (170 - 10);
        M_DrawSlider (220, 112, r);

// cursor
        M_DrawCharacter (200, 32 + enhanced_cursor*8, 12+((int)(realtime*4)&1));
}

void M_VidOpt_Key (int k)
{
	switch (k)
	{
	case K_ESCAPE:
	...
	case K_ENTER:
		m_entersound = true;
                M_VidOptAdjustSliders (1);
		return;
	case K_UPARROW:
		S_LocalSound ("misc/menu1.wav");
                enhanced_cursor--;
                if (enhanced_cursor < 0)
                        enhanced_cursor = ENHANCED_ITEMS-1;
		break;
	case K_DOWNARROW:
		S_LocalSound ("misc/menu1.wav");
                enhanced_cursor++;
                if (enhanced_cursor >= ENHANCED_ITEMS)
                        enhanced_cursor = 0;
		break;
	case K_LEFTARROW:
                M_VidOptAdjustSliders (-1);
