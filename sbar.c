...
// ammo counts
	for (i=0 ; i<4 ; i++)
	{
		...
		if (num[0] != ' ')
			Sbar_DrawCharacter ( (6*i+1)*8 - 9, -24, 18 + num[0] - '0');
		if (num[1] != ' ')
			Sbar_DrawCharacter ( (6*i+2)*8 - 9, -24, 18 + num[1] - '0');
		if (num[2] != ' ')
			Sbar_DrawCharacter ( (6*i+3)*8 - 9, -24, 18 + num[2] - '0');
			if (num[3] != ' ')
			Sbar_DrawCharacter ( (6*i+4)*8 - 9, -24, 18 + num[3] - '0');
	}
...
void Sbar_Draw (void)
{
	...
	if (sb_updates >= vid.numpages && !gl_clear.value && !scr_showedicts.value)
		return;
   ...
   // armor
	...
	// health
		...
		, cl.stats[STAT_HEALTH] <= 25);//was 136
