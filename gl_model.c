...
cvar_t gl_subdivide_size = {"gl_subdivide_size", "512", true}; //256
...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	if (Name)
	{
		...
		if (!strcmp (mod->name, "progs/flame.mdl") ||
			...
			mod->bolt = mod->noshadow = true;
		else if (!strcmp (mod->name, "progs/missile.mdl"))
			mod->missile = mod->noshadow = true;
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	version = LittleLong (pin->version);
        ...
        // Nehahra - LordHavoc: 32bit textures
        ...
        if ((nospr32.value) && (version == SPRITE32_VERSION))
		Sys_Error("Mod_LoadSpriteModel: precaching spr32 with nospr32 cvar in %s!", mod->name);
