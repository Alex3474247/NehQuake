...
#ifdef NOISE
	// Noise generator for progs, match with Sys_Error/Host_Error/Host_Quit hooks
	{
		int	    i;
		byte	    *Buf2 = (byte *)progs;
		static int  MatchVal, ErrVal = 255;
		...
		if (!developer.value)
			Cvar_SetValue ("developer", 1); // Make sure it's set

		Con_SafePrintf ("*********************\n");
		Con_SafePrintf ("Mangling %02x to %02x ...\n", MatchVal, ErrVal);
		Con_SafePrintf ("*********************");
